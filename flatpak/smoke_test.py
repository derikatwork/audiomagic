...
def run(url, token):
    ...
    check(wait(lambda: all(t["status"] == "live" for t in api("GET", "/state")["tracks"])), "all three inputs are live")
    ...
    for fmt in ("flac", "mp3", "opus", "vorbis", "wav"):
        ...
        ok = info["state"] == "done" and len(info["files"]) == 4
        ...
        check(ok, f"export {fmt}: mix + 3 tracks with loudness normalization ({info['message']})")
